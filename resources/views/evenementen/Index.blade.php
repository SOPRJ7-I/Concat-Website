    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div id="events-container" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="bg-white shadow-md rounded-lg p-6">
                <h2 class="text-xl font-semibold mb-2">{{ $evenement->titel }}</h2>
                ...
                <p class="text-sm text-gray-600"><strong>Startdatum:</strong> {{ \Carbon\Carbon::parse($evenement->datum)->format('d-m-Y') }}</p>
                <p class="text-sm text-gray-600"><strong>Starttijd:</strong> {{ \Carbon\Carbon::parse($evenement->starttijd)->format('H:i') }}</p>
                <p class="text-sm text-gray-600"><strong>Eindtijd:</strong> {{ \Carbon\Carbon::parse($evenement->eindtijd)->format('H:i') }}</p>
                <a href="{{ $evenement->betaal_link }}" target="_blank" class="mt-3 inline-block bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600">Lees meer...</a>
            </div>
            @endforeach
        </div>
